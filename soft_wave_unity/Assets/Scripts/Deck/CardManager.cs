...
public class CardManager : MonoBehaviour
{
    ...
    void Awake()
    {
        ...
        int[] initInven = { 0,1, 1, 1, 2, 2, 3, 3, 6 ,7, 7}; // 테스트 목적으로 임의의 덱을 소유하도록 함
        bool[] isDeck = { false,true, true, false, true, false, false, false, true, true,false };
    ...
    void Update()
    {
        ...
        if (!General.Instance.isPause)
        {
            ...
            if (Input.GetKeyDown(KeyCode.W)) UseCard(1);
            if (Input.GetKeyDown(KeyCode.E)) UseCard(2);
            if (Input.GetKeyDown(KeyCode.R)) UseCard(3);
        }
